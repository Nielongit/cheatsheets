...
function cookbook_download {
   echo
   echo "======"
   echo "download cookbook... ${version} .. ${cookbook}"
   if [ -d ~/${chef_ckb_dir}/${cookbook} ]; then
      mkdir -p ~/${chef_ckb_dir}
   fi
   cd ~/${chef_ckb_dir}
echo $(pwd)
echo "git clone --branch ${version} https://git.nexgen.neustar.biz/${cookbook_group}/${cookbook}.git"
   #git clone --branch v-${version} https://${USER}@git.nexgen.neustar.biz/oms-cookbooks/${cookbook}.git
	git clone --branch ${version} https://lcudneka@git.nexgen.neustar.biz/${cookbook_group}/${cookbook}.git
...
function cookbook_upload {
   ...
   if [ ${opt_force} ]; then
      ...
      knife cookbook upload ${cookbook} --freeze --force -V
   else
...
function download_environments_file {
   ...
   ################################
   cd ~/${chef_path}
   echo "... cloning environments files. chef_path:${chef_path}"
 #  git clone https://${USER}@git.nexgen.neustar.biz/oms/environments.git
...
function build_pattern_hash {
   ...
   IFS=","
   echo "... read in pattern file .${opt_inst}"
   while read line; do
      [[ $line =~ ^# ]] && continue
      [[ $line =~ ^$ ]] && continue
   ...
   if [ "${response}" == "y" ]; then
      for file in $(ls -1 ~/${env_out_dir} | tr '\n' ' '); do
         echo "... uploading ~/${env_out_dir}/${file}"
            result=$(knife environment from file ~/${env_out_dir}/${file}  2>&1 )
            echo ""
